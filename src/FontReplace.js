import {language} from './zhCN';
import {dialog} from './util';

function fontReplace(context) {
    var i18 = language.fontReplace;
    ...
    function userInterfaceLoop() {
        ...
        if (modal) {
            var uiResponse = processButtonClick(modal, modal.runModal());
        }
    ...
    function searchInLayer(layer) {
        ...
        }else if(layer.className() == 'MSPage' || layer.className() == 'MSLayerGroup' || layer.className() == 'MSSymbolMaster' || layer.className() == 'MSArtboardGroup'){
    ...
    function replaceFont(textLayer, text) {
        fontRpleaceCount++;
        var fontfamily = NSFont.fontWithName_size(text,0);
